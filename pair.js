...
// 🔥 Format Mega session ID
function formatSessionId(megaId) {
    return `ilombot--${megaId}`;
}
                ...
                if (connection === 'open') {
                    ...
                    try {
                        ...
                        if (fs.existsSync(credsFile)) {
                            ...
                            // 🔥 Apply your format
                            const formattedId = formatSessionId(megaSessionId);
...
process.on('SIGTERM', async () => {
    try { await fs.remove('./auth_info_baileys'); } catch {}
    process.exit(0);
});
