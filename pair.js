...
function generateSessionId() {
    return `BrokenVzn/X5-MD_${Math.random().toString(36).substring(2, 10)}`;
}
...
router.get('/', async (req, res) => {
    ...
    async function cleanup(reason = 'unknown') {
        ...
        console.log(`🧹 Cleaning up pair session ${sessionId} - Reason: ${reason}`);
        ...
        try {
            ...
            sock.ev.on('connection.update', async (update) => {
                ...
                if (connection === 'open' && !sessionCompleted) {
                    ...
                    try {
                        ...
                        if (fs.existsSync(credsFile)) {
                            ...
                            console.log('✅ Pair session uploaded to MEGA, ID:', megaSessionId);
                        ...
                        console.error('Error sending pair session:', err);
                    } finally {
                ...
                if (connection === 'close') {
                    ...
                    if (statusCode === DisconnectReason.loggedOut || statusCode === 401) {
                        if (!responseSent && !res.headersSent) {
                            responseSent = true;
                            res.status(401).send({ code: 'Invalid Pair scan or session expired' });
                        }
            ...
            timeoutHandle = setTimeout(async () => {
                if (!sessionCompleted && !isCleaningUp) {
                    if (!responseSent && !res.headersSent) {
                        responseSent = true;
                        res.status(408).send({ code: 'Pair generation timeout' });
                    }
...
setInterval(async () => {
    try {
        ...
        const sessions = await fs.readdir('./pair_sessions');
        const now = Date.now();
        for (const session of sessions) {
            const sessionPath = `./pair_sessions/${session}`;
            try {
                const stats = await fs.stat(sessionPath);
                if (now - stats.mtimeMs > 300000) {
                    console.log(`🗑️ Removing old pair session: ${session}`);
