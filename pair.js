...
function randomSessionSuffix(len = 32) {
...
router.get('/', async (req, res) => {
    ...
    const sessionSuffix = randomSessionSuffix(32);
    const sessionId = `ilombot--${sessionSuffix}`;
    const dirs = `./auth_info_baileys/${sessionId}`;
                ...
                if (connection === 'open') {
                    ...
                    try {
                        ...
                        if (fs.existsSync(credsFile)) {
                            const customName = `ILOM BOT/ILOM BOT_${sessionSuffix}`;
...
setInterval(async () => {
    try {
        ...
        for (const session of sessions) {
            const fullPath = `${baseDir}/${session}`;
            const stats = await fs.stat(fullPath);
            if (now - stats.mtimeMs > 10 * 60 * 1000) await fs.remove(fullPath);
        }
    } catch (e) {
